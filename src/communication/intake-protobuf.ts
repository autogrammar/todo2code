...
export function decodeIntakeEnvelope(bytes: Uint8Array, operation: 'command' | 'query'): IntakeEnvelope {
  try {
    const parsed = decodeDelimitedFields(bytes);
    const values = parsed.values;
    const unknownFields = parsed.unknownFields;
    const required = [1, 2, 3, 5, 6, 8, 9, 10];
    if (required.some((field) => !values.has(field))) throw new Error('Missing required envelope field');

    const payload = parsePayloadJson(values.get(10));
    const envelope = buildIntakeEnvelope(values, unknownFields, payload);
    assertIntakeEnvelope(envelope, operation);
    return envelope;
  } catch (error) {
...
export function decodeIntakeResult(bytes: Uint8Array): IntakeResult {
  try {
    ...
    return {
      ...
      payloadHash: strings.get(12) ?? '', diagnostic: parseOptionalJson(strings.get(13)),
      data: parseOptionalJson(strings.get(14)),
    };
  } catch (error) {
...
function decodeDelimitedFields(bytes: Uint8Array): {
  ...
  while (offset < bytes.length) {
    ...
    if (wire === 0) {
      ...
      if (field === 7) values.set(field, value);
      numbers.set(field, value);
      if (field !== 7) unknownFields.push(Buffer.from(bytes.slice(fieldStart, offset)).toString('base64'));
      continue;
    }
    if (wire === 2) {
      ...
      offset += length;
      if (field === 10 || field === 13 || field === 14 || (field >= 1 && field <= 12 && field !== 7)) {
        const value = Buffer.from(raw).toString('utf8');
        values.set(field, value);
        strings.set(field, value);
      } else {
        unknownFields.push(Buffer.from(bytes.slice(fieldStart, offset)).toString('base64'));
      }
      continue;
    }
    throw new Error(`Unsupported wire type ${wire}`);
  }
  return { values, strings, numbers, unknownFields };
}

function parsePayloadJson(raw: string | number | undefined): IntakeCommand | IntakeQuery {
  if (typeof raw !== 'string') throw new Error('Missing envelope payload');
  return JSON.parse(raw) as IntakeCommand | IntakeQuery;
}
...
function buildIntakeEnvelope(
  values: Map<number, string | number>,
  unknownFields: string[],
  payload: IntakeCommand | IntakeQuery,
): IntakeEnvelope {
  return {
    ...
    payloadHash: String(values.get(9)), payload,
