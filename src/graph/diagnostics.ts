...
function isRecordEvidenced(
  ...
  const implementedTarget = !hasCapabilityClaim(record) && hasImplementedTarget(record, context.implementedPaths);
  const documentedTarget = record.source.kind === 'changelog' && hasDocumentedTarget(record, context.documentedPaths);
