...
export interface MarkdownResponse {
  enrichments: MarkdownEnrichment[];
}

export interface CoveredBatch {
...
export class MarkdownAttemptError extends Error {
...
export async function enrichMarkdownRecords(
  client: OpenRouterClient,
  prompt: string,
  model: string,
  records: IntentRecord[],
  concurrency: number,
): Promise<{ enrichments: Map<string, MarkdownEnrichment>; responseByRecord: Map<string, LlmResponseMetadata>; responses: LlmResponseMetadata[] }> {
  ...
  const outcomes = await mapConcurrent(batches, concurrency, async (batch) => {
    try {
      const corrected = await enrichBatchCovering(client, prompt, model, batch);
  ...
  responses.push(...outcomes.flatMap((outcome) => (outcome.ok ? outcome.corrected.responses : outcome.responses)));
  const failed = outcomes.find((outcome) => !outcome.ok);
  if (failed && !failed.ok) throw new MarkdownAttemptError(failed.failure, responses);

  for (const outcome of outcomes) {
    if (!outcome.ok) continue;
    const { batch, corrected } = outcome;
    for (const record of batch) {
      const enrichment = corrected.enrichments.get(record.id);
      if (!enrichment) continue;
      enrichments.set(record.id, enrichment);
      const metadata = corrected.metadataByRecord.get(record.id);
      if (metadata) responseByRecord.set(record.id, metadata);
    }
  }
  return { enrichments, responseByRecord, responses };
}
...
export function promptRecord(record: IntentRecord): Record<string, unknown> {
  return {
    recordId: record.id,
...
export function validateEnrichments(values: MarkdownEnrichment[] | undefined, records: IntentRecord[]): Map<string, MarkdownEnrichment> {
...
export function enrichRecord(
  ...
  return buildRecord({
    ...
    extractor: record.source.kind === 'todo'
      ? 't2c/markdown-todo-openrouter@1'
      : 't2c/markdown-changelog-openrouter@1',
...
export function stageAudit(input: StageAuditInput): PipelineStageAudit {
  return {
