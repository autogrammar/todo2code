...
- Należy zachować jawną granicę LLM: OpenRouter może być wywoływany wyłącznie przez audytowalny orkiestrator NL (`src/extractors/nl-llm.ts`), semantyczne wzbogacanie TODO/CHANGELOG (`src/extractors/markdown-llm.ts`), komunikacji (`src/communication/llm.ts`), ekstrakcję dokumentacji, syntezę zadań i podsumowanie. Deterministyczne ekstraktory, linker, diagnostyka, diff i Intent vs Reality nie mogą importować klienta LLM; sprawdzają to `verify:no-llm` i `verify:modules`.
