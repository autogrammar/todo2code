...
| Dokumentacja → DSL przez LLM | `src/extractors/docs-llm.ts`, `src/llm/openrouter.ts` | `openrouter.test.ts`: structured output, target hints, współbieżność i wymuszony budżet chunków |
...
| Audyt runtime/LLM | `src/pipeline/run.ts`, `t2c.run/v1` | `pipeline.test.ts`: wersja, redacted config, fingerprint, metadane odpowiedzi i failed-run `require-llm` |
