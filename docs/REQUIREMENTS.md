...
| DSL + diagnostyka → `t2c.todo-proposal/v1` → `TODO.patch` | audytowana synteza w `src/synthesis/tasks-llm.ts`; deduplikacja z TODO, render/approval jeszcze planowane | `task-synthesis.test.ts`: sukces LLM, jawny fallback, `require-llm`, niepoprawne cytowania; pozostały zakres P0 w `TODO.md` |
