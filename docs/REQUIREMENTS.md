...
| Strukturalne wnioski `t2c.conclusion/v1` | typy i stabilne ID w `src/core/types.ts`, `src/core/id.ts`; JSON Schema i walidacja kontekstowa w `schemas/` oraz `src/core/schema.ts`; synteza jeszcze planowana | `grounded-contracts.test.ts`: kształt, cytowania, fingerprint, ID i metadane LLM/fallback |
| DSL + diagnostyka → `t2c.todo-proposal/v1` → `TODO.patch` | kontrakt propozycji i walidacja istnieją; synteza LLM, deduplikacja z TODO, render/approval jeszcze planowane | `grounded-contracts.test.ts`; pozostały zakres P0 w `TODO.md` |
