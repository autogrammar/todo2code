...
| Strukturalne wnioski `t2c.conclusion/v1` | typy i stabilne ID w `src/core/types.ts`, `src/core/id.ts`; JSON Schema i walidacja w `schemas/`/`src/core/schema.ts`; orkiestracja w `src/synthesis/tasks-llm.ts`, kontrakt i materializacja w `task-synthesis-{contract,materialize}.ts` | `grounded-contracts.test.ts`, `task-synthesis.test.ts`: kształt, cytowania, fingerprint, ID, metadane i odpowiedź LLM |
| DSL + diagnostyka → `t2c.todo-proposal/v1` → `TODO.patch` | orkiestracja w `src/synthesis/tasks-llm.ts`, ograniczony payload w `task-synthesis-payload.ts`, deduplikacja/kolejność w `src/synthesis/validation.ts`, renderer/audyt/approval w `src/synthesis/todo-patch.ts`; CLI/service/MCP/A2A i 5 SDK | `task-synthesis.test.ts`, `proposal-validation.test.ts`, `todo-patch.test.ts`, `cli-todo.test.ts`, `pipeline.test.ts`: LLM/fallback/timeout, cytowania, duplikaty/cykle, stabilny patch, stale/tampering, jawna zgoda i idempotentne apply |
| Diagnostyka → `t2c.code-change-plan/v1` → acceptance | `src/synthesis/code-change-plan.ts`, typy/walidacja w `src/core/{types,schema,id}.ts`, schematy JSON, CLI/MCP/A2A i SDK TypeScript | `code-change-plan.test.ts`: deterministyczne plany, provenance, path confinement, tampering oraz acceptance pass/fail po re-diagnose i przez CLI |
