...
test('Python package executes the local TypeScript reality runtime without a server', async () => {
  ...
  const script = [
    ...
    'assert runtime.version() == "todo2code 0.5.2"',
    ...
    'assert run["manifest"]["configuration"]["summaryLlm"] is False',
    'assert run["manifest"]["configuration"]["nlMode"] == "deterministic"',
    'assert run["manifest"]["configuration"]["markdownMode"] == "deterministic"',
